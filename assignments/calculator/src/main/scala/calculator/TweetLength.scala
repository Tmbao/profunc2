...
object TweetLength {
  ...
  def tweetRemainingCharsCount(tweetText: Signal[String]): Signal[Int] = Signal {
    ...
    MaxTweetLength - tweetLength(tweetString)
  }

  def colorForRemainingCharsCount(remainingCharsCount: Signal[Int]): Signal[String] = Signal {
    val remainingCharsInt: Int = remainingCharsCount()
    if (remainingCharsInt >= 15) "green"
    else if (0 <= remainingCharsInt && remainingCharsInt <= 14) "orange"
    else "red"
  }
